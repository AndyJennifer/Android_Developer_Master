...
再回想一下，在[崩溃分析](01|崩溃优化（上）：关于崩溃那些事儿.md)的时候我们就讲过一种获得所有线程堆栈的方法。它通过下面几个步骤实现。
...
在 Android 8.0 后，Android 虚拟机终于支持了 JVM 的[JVMTI机制](http://www.ibm.com/developerworks/cn/java/j-lo-jpda2/index.html)。Profiler 中内存采集等很多模块也切换到这个机制中实现，后面我会邀请“学习委员”鹏飞给你讲讲 JVMTI 机制与应用。使用它可以获得的信息非常丰富，包括内存申请、线程创建、类加载、GC 等，有大量的应用场景。
...
我在评论中发现很多同学对监控 Thread 的创建比较感兴趣，今天我们的[Sample](https://github.com/AndroidAdvanceWithGeektime/Chapter06-plus)是如何监控线程的创建。在实践前，给你一些可以参考的链接。

- [Android 线程的创建过程](http://www.jianshu.com/p/a26d11502ec8)
