...
因为每个应用基本都会用到 JSON，所以每个大厂也基本都有自己的“轮子”。例如 Android 自带的 JSON 库、Google 的[Gson](<https://github.com/google/gson>）、阿里巴巴的[Fastjson](https://github.com/alibaba/fastjson/wiki/Android%E7%89%88%E6%9C%AC0)、美团的[MSON](https://tech.meituan.com/MSON.html)。
...
在[专栏第 9 期](09|IO优化（上）：开发工程师必备的IO优化知识.md)中我讲过，应用程序、文件系统或者磁盘都可以导致文件损坏。
...
线上我们有时候会发现在遍历某个文件夹时，会出现卡顿或者 ANR。在[专栏第 10 期](10|IO优化（中）：不同IO方式的使用场景是什么.md)我也讲过，文件遍历的耗时跟文件夹下的文件数量有关。曾经我们也出现过一次 bug 导致某个文件夹下面有几万个文件，在遍历这个文件夹时，用户手机直接重启了。需要注意的是文件遍历在 API level 26 之后建议使用[FileVisitor](https://developer.android.com/reference/java/nio/file/FileVisitor)替代 ListFiles，整体的性能会好很多。
